...
import '@_openzeppelin_/contracts/utils/Strings.sol';
import '@_openzeppelin_/contracts/utils/math/SafeMath.sol';
import '@_openzeppelin_/contracts/utils/math/SignedSafeMath.sol';
...
library NFTDescriptor {
    ...
    function constructTokenURI(ConstructTokenURIParams memory params) public pure returns (string memory) {
        bytes memory name = generateName(params, feeToPercentString(params.fee));
        bytes memory descriptionPartOne =
            generateDescriptionPartOne(
                escapeQuotes(params.quoteTokenSymbol),
                escapeQuotes(params.baseTokenSymbol),
                addressToString(params.poolAddress)
            );
        bytes memory descriptionPartTwo =
    ...
    function escapeQuotes(bytes memory symbol) internal pure returns (bytes memory) {
        uint8 quotesCount = 0;
        for (uint8 i; i < symbol.length;) {
            if (symbol[i] == '"') {
                quotesCount++;
            }
            unchecked { ++i; }
        ...
        if (quotesCount > 0) {
            bytes memory escapedBytes = new bytes(symbol.length + (quotesCount));
            uint256 index;
            for (uint8 i; i < symbol.length;) {
                if (symbol[i] == '"') {
                    escapedBytes[index++] = '\\';
                }
                escapedBytes[index++] = symbol[i];
                unchecked { ++i; }
            }
            return escapedBytes;
        }
    ...
    function generateDescriptionPartTwo(
        ...
        bytes memory baseTokenSymbol,
        bytes memory quoteTokenAddress,
        bytes memory baseTokenAddress,
    ...
    function generateDecimalString(DecimalStringParams memory params) private pure returns (bytes memory) {
    ...
    function tickToDecimalString(
        ...
        if (tick == (TickMath.MIN_TICK / tickSpacing) * tickSpacing) {
            return bytes(!flipRatio ? 'MIN' : 'MAX');
        } else if (tick == (TickMath.MAX_TICK / tickSpacing) * tickSpacing) {
            return bytes(!flipRatio ? 'MAX' : 'MIN');
        } else {
    ...
    function feeToPercentString(uint24 fee) internal pure returns (string memory) {
        ...
        return string(generateDecimalString(params));
    }

    function addressToString(address addr) internal pure returns (bytes memory) {
        return bytes(HexStrings.toHexString((uint256(uint160(addr))), 20));
    }

    function generateSVGImage(ConstructTokenURIParams memory params) internal pure returns (bytes memory svg) {
    ...
    function scale(
        ...
        return bytes((n.sub(inMn).mul(outMx.sub(outMn)).div(inMx.sub(inMn)).add(outMn)).toString());
    }

    function tokenToColorHex(uint256 token, uint256 offset) internal pure returns (bytes memory str) {
        return bytes( HexStrings.toHexStringNoPrefix( (token >> offset) , 3) );
    }
